...
local function MoveTo9(point, indent, seen)
	...
	if val == 9 then
		print(string.rep(" ", indent) .. "reached 9")
		local isUnique = true
		for _, p in ipairs(seen) do
			if p.i == point.i and p.j == point.j then
				isUnique = false
				break
			end
		end
		table.insert(seen, point)
		if isUnique then
			return 1
		end
		return 0
...
for _, start in pairs(zeroPositions) do
	...
	local score = MoveTo9(start, 0, {})
	if score > 0 then
		table.insert(trails, { i = start.i, j = start.j, score = score })
	end
end
